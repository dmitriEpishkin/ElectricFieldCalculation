...
namespace Nordwest.Wpf.Controls
{
    ...
    public static class CohenSutherland
    {
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private static byte ComputeOutCode(this Rect extents, double x, double y)
        {
            ...
            if (y < extents.Top)       // above the clip window
                code |= TOP;
            else if (y > extents.Bottom)         // below the clip window
                code |= BOTTOM;
        ...
        public static List<Point> CohenSutherlandLineClip(this Rect extents,
                               Point p0, Point p1)
        {
            double x0 = p0.X;
            double y0 = p0.Y;
            ...
            byte outcode0 = extents.ComputeOutCode(x0, y0);
            byte outcode1 = extents.ComputeOutCode(x1, y1);
            bool accept = false;
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (outcodeOut == outcode0)
                    {
                        x0 = x;
                        y0 = y;
                        outcode0 = extents.ComputeOutCode(x0, y0);
                    }
                    else
                    {
                        x1 = x;
                        y1 = y;
                        outcode1 = extents.ComputeOutCode(x1, y1);
                    }
                }
            }
